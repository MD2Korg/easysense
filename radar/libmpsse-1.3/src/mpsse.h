...
char Version(void);
int FastReadx(struct mpsse_context *mpsse, int size);
int FastReady(struct mpsse_context *mpsse, char *data, int size);
...
int fast_build_block_bufferMod(struct mpsse_context *mpsse, uint8_t cmd, unsigned char *data, int size, int *buf_size,int idx);
int FastWriteMod(struct mpsse_context *mpsse, char *data, int *size, int numCommands, int *rxSize);
#endif
