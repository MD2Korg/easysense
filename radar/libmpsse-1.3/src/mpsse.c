 ...
 */
struct mpsse_context *MPSSE(enum modes mode, int freq, int endianess,int latencyTimer)
{
	int i = 0;
	...
	{
		if((mpsse = Open(supported_devices[i].vid, supported_devices[i].pid, mode, freq, endianess, IFACE_A, NULL, NULL, latencyTimer)) != NULL)
		{
			if(mpsse->open)
			{
...
/*
 * Opens and initializes the FTDI device indexed.
 * 
 * @mode      - Mode to open the device in. One of enum modes.
 * @freq      - Clock frequency to use for the specified mode.
 * @endianess - Specifies how data is clocked in/out (MSB, LSB).
 * @index     - Specifies the index of the FTDI device being accessed, starts from 0.
 * Returns a pointer to an MPSSE context structure. 
 * On success, mpsse->open will be set to 1.
 * On failure, mpsse->open will be set to 0.
 */
struct mpsse_context *MPSSEMod(enum modes mode, int freq, int endianess,int latencyTimer,int index)
{
	int i = 0;
	struct mpsse_context *mpsse = NULL;
	...
	for(i=0; supported_devices[i].vid != 0; i++)
	{
		if((mpsse = OpenMod(supported_devices[i].vid, supported_devices[i].pid, mode, freq, endianess, IFACE_A, NULL, NULL, latencyTimer,index)) != NULL)
		{
			if(mpsse->open)
			{
				mpsse->description = supported_devices[i].description;
 ...
 * On failure, mpsse->open will be set to 0.
 */
struct mpsse_context *Open(int vid, int pid, enum modes mode, int freq, int endianess, int interface, const char *description, const char *serial, int latencyTimer)
{
	return OpenIndex(vid, pid, mode, freq, endianess, interface, description, serial, 0, latencyTimer);
}
/* 
 * Open device by VID/PID and index
 *
 * @vid         - Device vendor ID.
 * @pid         - Device product ID.
 * @mode        - MPSSE mode, one of enum modes.
 ...
 * @description - Device product description (set to NULL if not needed).
 * @serial      - Device serial number (set to NULL if not needed).
 * @index	- Index of the FTDI device being accesssed, starts from 0.
 *
 * Returns a pointer to an MPSSE context structure. 
 * On success, mpsse->open will be set to 1.
 * On failure, mpsse->open will be set to 0.
 */
struct mpsse_context *OpenMod(int vid, int pid, enum modes mode, int freq, int endianess, int interface, const char *description, const char *serial, int latencyTimer,int index)
{
	return OpenIndex(vid, pid, mode, freq, endianess, interface, description, serial, index, latencyTimer);
}
/* 
 ...
 * On failure, mpsse->open will be set to 0.
 */
struct mpsse_context *OpenIndex(int vid, int pid, enum modes mode, int freq, int endianess, int interface, const char *description, const char *serial, int index, int latencyTimer)
{
	int status = 0;
	struct mpsse_context *mpsse = NULL;
	...
	if(mpsse)
	{
		...
		if(ftdi_init(&mpsse->ftdi) == 0)
		{
			...
			if(ftdi_usb_open_desc_index(&mpsse->ftdi, vid, pid, description, serial, index) == 0)
			{
				...
				status |= ftdi_set_latency_timer(&mpsse->ftdi,latencyTimer);
...
/* Performs a read. For internal use only; see Read() and ReadBits(). */
void InternalReadMod(struct mpsse_context *mpsse,unsigned char *buf, int size)
{
...
void ReadMod(struct mpsse_context *mpsse, unsigned char *buf,int size)

{
	InternalReadMod(mpsse, buf, size);
}
...
void TransferMod(struct mpsse_context *mpsse, char *data, unsigned char *buf, int size)
{
