...
int FastWriteMod(struct mpsse_context *mpsse, char *data, int *size, int numCommands, int *rxSize)
{
	...
	if(is_valid_context(mpsse))
	{
		if(mpsse->mode)
		{
			...
			for(i=0;i<numCommands;i=i+1)
			{
			
			fast_build_block_bufferMod(mpsse, mpsse->tx, (unsigned char *) (data + n), size[i], &buf_size,buf_size);
			
			fast_build_block_bufferMod(mpsse, mpsse->rx,NULL, rxSize[i], &buf_size,buf_size);
...
{
	int n = 0, rxsize = size, data_size = 0;
